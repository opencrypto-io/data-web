...
var Counter = {
  ...
  view: function () {
    return Object.keys(defs).map(function (col) {
      return m(".tile.is-parent", m("article.tile.is-child.box.has-text-centered", [m("span.col", "" + col), m("div.count", counts[col] || m("span", { style: "color: gray;" }, "?"))]));
    });
  }
};
var Contributors = {
...
var DataSample = {
  ...
  view: function () {
    return m("div", [m("div", [m(".level", { style: "margin-bottom: 0.7em;" }, [m('.level-item', [m("label", { style: "padding-right: 0.5em;" }, "Model: "), m(".select", m("select", { type: "text", value: model, oninput: m.withAttr("value", setModel), style: "background-color: rgba(255, 255, 255, 0.3); border: none;" }, Object.keys(defs).map(function (k) {
      return m("option", { id: defs[k].model }, defs[k].model);
    })))]), m('.level-item', [m("label", { style: "padding-right: 0.5em;" }, "Id: "), m("input.input", { type: "text", value: sampleId, oninput: m.withAttr("value", setId), style: "background-color: rgba(255, 255, 255, 0.3); border: none;" })])]), m(".level", { style: "margin-bottom: 0.7em;" }, [m('.level-item', [m("label", { style: "padding-right: 0.5em;" }, "Query: "), m("input.input", { type: "text", value: query, oninput: m.withAttr("value", setQuery), style: "background-color: rgba(255, 255, 255, 0.3); border: none;" })])])]), m("pre", m("code", m.trust(formattedSource))), m("div", { style: "padding-top: 0.5em;" }, [m(".level", [m(".level-left", [m(".control.level-item", [m("span", "Format: "), m("label.radio", [m("input", { oninput: m.withAttr("value", setFormat), type: "radio", name: "format", value: "yaml", checked: format === "yaml" }), " YAML"]), m("label.radio", [m("input", { oninput: m.withAttr("value", setFormat), type: "radio", name: "format", value: "json", checked: format === "json" }), " JSON"])])]), m(".level-right", [m(".level-item", m("a", { href: "https://github.com/opencrypto-io/data/blob/master/db/projects/" + sampleSourceProject + "/project.yaml", style: "color: white;" }, [m("i.far.fa-file"), m("span", " Source file")])), m(".level-item", m("a", { href: "https://github.com/opencrypto-io/data/edit/master/db/projects/" + sampleSourceProject + "/project.yaml", style: "color: white;" }, [m("i.far.fa-edit"), m("span", " Edit")]))])])])]);
  }
};
var LastCommit = {
  oninit: async function () {
  ...
  view: function () {
    ...
    return m("div", ["Last commit: ", m("a", { href: "https://github.com/opencrypto-io/data/commits/master" }, metadata.commit), " (" + moment(metadata.time).fromNow() + ")"]);
  }
};
...
//# sourceMappingURL=/webapp.3433705f.map